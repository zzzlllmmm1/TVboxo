...
function wpLoadHtml(init) {
    const res = webParse(wpurl + '<<eval:"ok"?load=html' + (init ? '&init=1' : ''), wpHtml);
    console.log('wpLoadHtml: ' + res);
}
...
async function detail(tid) {
    ...
    const data = webParse(wpurl + '<<eval:getData(tvbox.getPostBody())', urls);
    
    vod.vod_play_url = data.replaceAll(/\n/g, "#");
...
async function play(flag, id, flags) {
    const pid = webParse(wpurl + '<<eval:getPid()', '');
    return JSON.stringify({
        parse: 0,
        url: id + '?pid=' + pid,
...
async function search(wd, quick) {
    const pid = webParse(wpurl + '<<eval:getPid()?load=html', wpHtml);
    
    const url = root + '/video/search?q=' + encodeURIComponent(wd) + '&pid=' + pid;
    console.log(url);
